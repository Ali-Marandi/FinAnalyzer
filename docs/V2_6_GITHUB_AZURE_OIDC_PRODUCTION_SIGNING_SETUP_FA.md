# راه‌اندازی گام‌به‌گام GitHub OIDC و محیط `production-signing` — FinAnalyzer v2.6.0
...
۲. پس از عبور تست staging، یک tag immutable واقعی ایجاد کنید؛ مانند `v2.6.0`. این کار باید پس از review تغییرات و build evidence انجام شود.
...
```powershell
Get-FileHash .\FinAnalyzer_Enterprise_v2_6.exe -Algorithm SHA256
Get-AuthenticodeSignature .\FinAnalyzer_Enterprise_v2_6.exe |
